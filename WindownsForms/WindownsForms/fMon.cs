...
namespace WindowsForms
{
    public partial class fMon : Form
    {
        ...
        {
            string sql;
            sql = "SELECT Id,TenMon FROM MONHOC where TrangThai = 0";
            dtTable = dtBase.DataReader(sql);
            dgvMon.DataSource = dtTable;
            dgvMon.Columns[0].HeaderText = "ID";
            dgvMon.Columns[1].HeaderText = "Môn Học";
            dgvMon.Columns[0].Width = 100;
            dgvMon.Columns[1].Width = 210;
        ...
        private void dgvMon_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            txbID.Text = dgvMon.CurrentRow.Cells[0].Value.ToString();
            txbName.Text = dgvMon.CurrentRow.Cells[1].Value.ToString();
            txbName.Focus();
        ...
        private void btnNew_Click(object sender, EventArgs e)
        {
            ...
            txbID.Text = dtBase.SinhMaTuDong("MONHOC", "Id");
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            sql = "Select * FROM MONHOC where TenMon =N'" + txbName.Text +"'";
            if(dtBase.CheckKey(sql))
            {
                sql = "UPDATE MONHOC SET  TrangThai = 0 where TenMon =N'" + txbName.Text +"'";
                dtBase.DataChange(sql);
            }
            else
            {
                sql = "INSERT INTO MONHOC(TenMon)" + " VALUES (N'" + txbName.Text.Trim() + "')";
                dtBase.DataChange(sql);
            }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                sql = "UPDATE MONHOC SET TrangThai = 1 where Id=" + txbID.Text.Trim();
                dtBase.DataChange(sql);
